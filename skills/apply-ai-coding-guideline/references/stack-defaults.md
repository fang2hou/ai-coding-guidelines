...
| Lint            | oxlint + oxlint-tsgolint (typeAware + typeCheck via .oxlintrc.json; node plugin loaded with deliberate rule choices) | ruff (E, F, I, B, UP, SIM)          | golangci-lint (.golangci.yml tailored per project, never enable-all; context and error emphasis groups; relaxed per-path rules for \_test.go) | clippy -D warnings    |
