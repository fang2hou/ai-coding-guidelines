...
10. 检查产生的 diff。
11. 确认没有引入误加文件或无关变更。
12. 对重大的架构、依赖、安全或行为变更作出说明。

不要仅仅因为 Agent 偏好另一种风格,就重写正常工作的代码。
...
仓库中的指南与项目文档,始终优先于记忆中的信息。

在实质性工作期间,持久化 Agent 应定期回看当前项目指南。
...
仓库中已有现行规则时,不要依赖记忆中的摘要。

## 硬性规则速查

以下行为均被禁止。每一条链接到定义该规则的文档;规则变更时,保持本列表与那些文档同步。

1. 未经用户明确批准,替换强制性标准化工具。([core-principles](../principles/core-principles.md))
2. 使用 npm 或 yarn 而非 pnpm。([typescript](../toolchain/typescript.md))
3. 将 ESLint 而非 oxlint 用作项目常规 Lint 工具。([typescript](../toolchain/typescript.md))
...
8. 无正当理由且未经批准,用无关的 Conventional Commits 校验器取代 Cocogitto。([git](../toolchain/git.md))
9. 未理解 lint 或校验问题,就忽略它们。([core-principles](../principles/core-principles.md))
...
19. 在跨语言 UI 工作中产出低质量的直译。([language-policy](../practices/language-policy.md))
20. 不必要地修改 shadcn/ui 组件内部实现。([shadcn-ui](../libraries/shadcn-ui.md))
21. 对必需的 shadcn/ui 组件修改不做文档记录。([shadcn-ui](../libraries/shadcn-ui.md))
...
25. 把 CI 通过当作理解实现是否正确的替代品。([change-discipline](../practices/change-discipline.md))
