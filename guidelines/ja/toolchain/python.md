...
依存関係の定義は `pyproject.toml` に記述する。は生成物であり、手で編集しない。

単体のスクリプトでは、PEP 723 のインラインスクリプトメタデータで依存関係を宣言し、で実行する。
