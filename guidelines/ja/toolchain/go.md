...
設定は `.golangci.yml` としてコミットし、プロジェクトの重点に合わせて調整する。`enable-all` は使わない。すべてを有効にすると、遅くて独自の見解の強いチェックまで入り、本当に重要な指摘が埋もれてしまう。
...
`_test.go` ファイルには、`.golangci.yml` のパス単位の除外で緩いリントルールを適用する。テストは本番コードと同じリントの厳格さではなく、可読性と意図の明確さを優先する。エラーのラップや重複コードのスタイル検査はテストではオフにする：
...
Go に公式の標準プロジェクトレイアウトは存在しない。`golang-standards/project-layout` リポジトリはコミュニティによる参考例であり、公式の標準ではない。公式ガイド Organizing a Go module も、基本のパッケージをモジュールのルートに置く構成から説明を始めている。
