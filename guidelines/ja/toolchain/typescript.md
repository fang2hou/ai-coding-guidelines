...
コードが実際に動くプラットフォームに応じてプラグインを読み込む。デフォルトだけでなく、Node.js 上で動くコードでは `.oxlintrc.json` で `node` プラグインを有効化し、そのルールを意図をもってオンにする。CommonJS 時代のパターン防止ルールも含める一方、プロジェクトの意図と衝突する制限系ルールは除外する（同期ファイルアクセスが意図的な CLI では `no-sync` をオフのままにするなど）。
