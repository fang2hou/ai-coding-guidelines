...
すべてのプロジェクトは、次の 4 つのドキュメントを維持しなければならない。出発点として、配下のテンプレートを使用すること。
...
プロジェクトの理解と利用に必要な情報を含むべきである。含める情報の例は次のとおりである。
...
プロジェクトで確定したプロダクト/UI の言語ポリシー([言語ポリシー](language-policy.md)を参照)も、適切で目につきやすいプロジェクトドキュメント(できれば README または DEVELOPMENT ドキュメント)に記録すべきである。
...
人間と AI エージェントのどちらも、開発の進め方を理解できる内容にすること。
...
AI が生成または支援したプルリクエストでは、説明文に次の内容を明確に含めること。
...
意味のあるアーキテクチャ上の境界を持つプロジェクトは、を維持すべきである。
