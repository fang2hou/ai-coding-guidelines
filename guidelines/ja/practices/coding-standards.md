...
コメントをほとんど必要としないコードを書くこと。命名・構造・分割が意味を担い、コメントはそれらでは表現できない事柄だけを扱う。

- 理由を書き、処理内容の言い換えを書かない。
- ロジックを一歩ずつ解説しない。解説が必要になる箇所は、まず再構成すること。
- 削除済みの振る舞いや過去の意図を説明するコメントを残さない。
- 設定ファイルはコメントなしに保ち、根拠はプルリクエストまたはプロジェクトのドキュメントに記録する。
...
後から来る読者のために書くこと。関数は関心事ごとにまとめ、ロジックのレベルを分離し、ファイルの並び順そのもので流れを語らせる。主経路を先に、ヘルパーを後に置く。
...
正確さと可読性を優先し、計測していない速度のために構造を犠牲にしないこと。

- 回避できる割り当て・コピー・再計算をしない。
- O(n) 回実行されるループの中で O(n) の処理をしない。
- 断片的なマイクロ最適化の前に、アルゴリズムとデータ構造を修正する。
- 計測で特定したホットパスのみを、証拠に基づいて最適化する。
