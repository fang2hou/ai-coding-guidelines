...
- 减少开发成本、维护成本、视觉不一致、升级冲突,以及 AI 生成的重复组件。
- 官方变体与配置机制覆盖大部分定制需求,无需改动组件内部。

## 代价

- 复制进来的组件可能与上游漂移;改动组件内部会带来升级风险。
- 需要持续的纪律才能让组件保持贴近官方实现。
...
- 在合理范围内尽可能使用 shadcn/ui 组件。
- 创建自定义组件前,先确认 shadcn/ui 是否已提供合适的基础组件或现成组件。
- 优先使用现成组件、现成组件的组合、官方变体和官方配置机制,避免不必要的自定义实现。
- 目的在于减少开发成本、维护成本、视觉不一致、升级冲突,以及 AI 生成的重复组件。

### 避免不必要的修改

- 下载的组件保持贴近官方实现。
- 凡是能通过主题变量、Tailwind 配置、组件组合、现有组件 API、标准变体、包装组件或受支持的设计令牌实现的改动,不要通过修改组件内部来实现。
...
- 将这些信息保存在仓库内的 Markdown 文档中,让后续开发者与 AI Agent 在升级 shadcn/ui 时能够理解这些偏离。

### shadcn MCP

- 开发环境支持 MCP 时,优先使用官方或合适的 shadcn MCP 集成。
- 具备 MCP 能力的用户在使用 shadcn/ui 但尚未配置 MCP 集成时,Agent 可协助其完成配置。
