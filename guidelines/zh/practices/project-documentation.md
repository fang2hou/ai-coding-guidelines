...
- 上手章节与 Learn More 之间是一块可自由拼搭的展示区，只用项目需要的板块：`Concepts`——读者需要掌握的少数几个概念；`Features`——项目做什么、为何重要、与替代品有何差异；`Performance`——一句话性能主张，配上证明它的图表或表格、测量环境和方法论链接，没有数字就不要下结论；`Experiments`——实验内容，有则展示。板块顺序取决于项目最想证明什么；每个板块配一个标准标题和一个 emoji。
- 一张“目标 → 阅读”表，链接到下文的详细文档。
- 明确声明的许可证。

章节标题用标准、简短、一眼能认出的名字——`Quick Start`、`Usage`、`Concepts`、`Features`、`Performance`、`Experiments`、`Learn More`、`License`——并在每个标题前加一个含义固定的 emoji：上手章节用 🚀，概念用 💡，特性用 ✨，性能用 ⚡，实验用 🧪，延伸阅读用 📚，许可证用 📄。emoji 保持少而统一；项目标题与正文不使用 emoji。
