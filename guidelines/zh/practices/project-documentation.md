...
- 概念与特性：读者需要的少数几个想法，以及项目与替代品的差异——每条一行，放在上手章节之后。
- 一张“目标 → 阅读”表，链接到下文的详细文档。
- 明确声明的许可证。

章节标题保持标准、简短、一眼可辨——`Quick Start`、`Usage`、`Concepts & Features`、`Learn More`、`License`——并为每个标题前缀一个语义稳定的 emoji：上手章节用 🚀，概念与特性用 ✨，延伸阅读用 📚，许可证用 📄。项目标题至多携带一个辨识性 emoji。emoji 集合保持小而一致，正文文字绝不使用 emoji。
